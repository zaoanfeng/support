...
public class ServiceManager {
	 ...
	 * @throws TimeoutException
	 */
	public boolean install(String windowsFile, String linuxFile, String serviceName, int timeout) throws IOException, InterruptedException, TimeoutException {
		if (System.getProperty("os.name").toLowerCase().startsWith("window")) {
	 ...
	 * @throws TimeoutException
	 */
	public boolean uninstall(String windowsFile, String linuxFile, String serviceName, int timeout) throws IOException, InterruptedException, TimeoutException {
		if (System.getProperty("os.name").toLowerCase().startsWith("window")) {
	 ...
	 * @throws TimeoutException
	 */
	private boolean startOfLinux(String serviceName, int timeout) throws IOException, InterruptedException, TimeoutException {
		return shellOfLinux( new String[] {"service", "-c", "/sbin/service " + serviceName.trim() + " start"}, timeout);
	}
	
	/**
	 * windows下启动服务
	 * @param serviceName 服务名
	 * @param timeout 启动服务最大等待时长 （秒）
	 * @return
	 * @throws IOException
	 * @throws InterruptedException
	 * @throws TimeoutException
	 ...
	 * @throws TimeoutException
	 */
	private boolean stopOfWindows(String serviceName, int timeout) throws IOException, TimeoutException, InterruptedException {
	 ...
	 * @throws TimeoutException
	 */
	private boolean installOfWindows(String exec, String serviceName, int timeout) throws IOException, InterruptedException, TimeoutException {
	 ...
	 * @throws TimeoutException
	 */
	private boolean installOfLinux(String exec, String serviceName, int timeout) throws IOException, InterruptedException, TimeoutException {
		// linux下拷贝 "sh", "-c", "/bin/cp /data1 /data2" 
		String cmd[] = {
				"sh", "-c", "/bin/cp -f " + exec + " /etc/init.d/" + serviceName.trim(), 
				"chkconfig --add " + serviceName.trim(), 
				"chkconfig --level 2345 " + serviceName.trim() + " on" 
				};
	 ...
	 * @param serviceName 服务名
	 * @param timeout 卸载服务最大等待时长 （秒）
	 * @return
	 * @throws IOException
	 * @throws InterruptedException
	 * @throws TimeoutException
	 */
	private boolean uninstallOfWindows(String exec, String serviceName, int timeout) throws IOException, InterruptedException, TimeoutException {
		String path = exec.substring(0, exec.lastIndexOf(File.separator));
		String cmd = exec.substring(exec.lastIndexOf(File.separator) + 1);
		Process process = Runtime.getRuntime().exec("cmd /c " + cmd + " remove " + serviceName.trim(), null, new File(path));
		Worker worker = new Worker(process);
        worker.start();
        try {
            worker.join(timeout * SECOND);
	 ...
	 * @throws TimeoutException
	 */
	private boolean uninstallOfLinux(String exec, String serviceName, int timeout) throws IOException, InterruptedException, TimeoutException {
